    ...
    public class MissionTestChatCommand : IChatCommandListDefinition
    {
        ...
        private void ObjectiveTest()
        {
            _builder.Create("objtest")
                .Description("(DM) Mission objective test: /objtest kill <tag> <count> | boss <tag> | destroy <tag> | reach <tag> [radius] | retrieve <itemTag> <extractTag> | protect <tag> [seconds] | rescue <npcTag> <extractTag> | escort <npcTag> <extractTag> | slice <tag> [ticks] | eliminate | extract <itemTag> [extA] [extB] | hold <zoneTag> [ticks] | end")
                ...
                .Action((user, target, location, args) =>
                {
                    ...
                    if (args.Length == 0)
                    {
                        SendMessageToPC(user, "Usage: /objtest kill <tag> <count> | boss <tag> | destroy <tag> | reach <tag> [radius] | retrieve <itemTag> <extractTag> | protect <tag> [seconds] | rescue <npcTag> <extractTag> | escort <npcTag> <extractTag> | slice <tag> [ticks] | eliminate | extract <itemTag> [extA] [extB] | hold <zoneTag> [ticks] | end");
                        return;
                    }
                    ...
                    switch (args[0].ToLower())
                    {
                        ...
                        case "slice":
                            ...
                            break;
                        case "eliminate":
                            // Elimination mode/lives are configured on the Side match (/sidetest start ...).
                            Mission.AddObjective(area, new EliminateObjective(area));
                            break;
                        case "extract":
                            if (args.Length < 2)
                            {
                                SendMessageToPC(user, "Usage: /objtest extract <itemTag> [extractTagA] [extractTagB]");
                                return;
                            }
                            var extractA = args.Length >= 3 ? args[2] : "EXTRACT_A";
                            var extractB = args.Length >= 4 ? args[3] : "EXTRACT_B";
                            Mission.AddObjective(area, new ExtractionRaceObjective(area, args[1],
                                new Dictionary<string, string> { { "A", extractA }, { "B", extractB } }));
                            break;
                        case "hold":
                            if (args.Length < 2)
                            {
                                SendMessageToPC(user, "Usage: /objtest hold <zoneTag> [targetTicks]");
                                return;
                            }
                            var targetTicks = args.Length >= 3 && int.TryParse(args[2], out var tt) ? tt : 10;
                            Mission.AddObjective(area, new HoldPointObjective(area, args[1], targetTicks));
                            break;
                        case "end":
                            Mission.EndRun(area);
                            break;
                        default:
                            SendMessageToPC(user, "Unknown type. Use: kill | boss | destroy | reach | retrieve | protect | rescue | escort | slice | eliminate | extract | hold | end.");
                            break;
                    }
                });
        }

...
### C. Inert dead code — EXCISED (done)
The parked research and food code has now been fully removed (grep-verified,
not build-verified):
- **Research internals** deleted: the Research window trio, `ResearchJob`
  entity, all research-only members of `Craft.cs`, the `RecipesUIMode.Research`
  branches in `RecipesViewModel.cs`, the `Skill.Cache` researchable plumbing,
  and the `RecipesUIMode.Research` / `GuiWindowType.Research` /
  `ScriptName.OnResearchTerminal` values. `CalculateResearchCost` and
  `CalculateBlueprintCraftCreditCost` were KEPT (Engineering blueprint path).
- **Food internals** deleted: `FoodEffectData.cs`, all null-guarded food reads
  (`Stat.cs`, `Recast.cs`, `Skill.cs`, `NaturalRegeneration.cs`,
  `RestStatusEffectDefinition.cs`), `ReapplyFoodHP`, `Player.TemporaryFoodHP`,
  and `StatusEffectType.Food`.
- **CraftViewModel** dead Smithery/Fabrication/Cooking switch cases removed
  (cosmetic-tidy commit).

Left as harmless inert enum-value orphans (removing them is churn with no
gain): `StatusEffectType`/`EffectIconType` gaps, `ItemPropertyType.FoodBonus`
(still generated by Engineering enhancement code — NOT dead),
`StructureType.ResearchTerminal`, `RecipeType.ResearchTerminal`,
`PerkType.Research`/`ResearchProjects`/`ScientificNetworking`, and the
`IsShownInResearchMenu` skill-attribute flag (removing it would churn all 38
`[Skill]` declarations).
